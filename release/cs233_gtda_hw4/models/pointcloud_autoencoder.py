...
class PointcloudAutoencoder(nn.Module):
    ...
    def train_for_one_epoch(self, loader, optimizer, device='cuda'):
        ...
        for data in loader:
            ...
            loss = chamfer_loss(recons, points).mean()
            loss.backward()
            optimizer.step()
            loss_meter.update(loss.item(), points.size(0))
        return loss_meter.avg
    
    def train_with_seg_one_epoch(self, loader, optimizer, num_classes, device="cuda"):
        self.train()
        ...
        for data in loader:
            ...
            part_masks = torch.nn.functional.one_hot(part_masks.long(), num_classes=num_classes).to(device).float()
            ...
            seg_loss = self.seg_lossfn(point_masks.reshape(-1,num_classes), part_masks.reshape(-1,num_classes)).mean()
    ...
    def reconstruct(self, loader, device='cuda'):
        ...
        return torch.vstack(point_clouds), torch.vstack(reconstructed_pcds), shape_name
        
    @torch.no_grad()
    def reconstruct_with_seg(self, loader, device='cuda'):
        self.eval()
        point_clouds = []
        reconstructed_pcds = []
        part_masks = []
